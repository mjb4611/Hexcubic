...
class Down_D5 extends Component {
    constructor(props) {
        super(props);
    ...
    render() {
        const { hexagons } = this.state;
        return (
          <Layout className="tiles" size={{ x: 10, y: 10}} flat={false} spacing={1.01} origin={{ x: 50, y: -63 }}>
{
    hexagons.map((hex, i) => (
      <Hexagon
        key={i}
