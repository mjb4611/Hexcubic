...
class Up_ShadeRight extends Component {
    constructor(props) {
        super(props);
    ...
    render() {
        const { hexagons } = this.state;
        return (
          <Layout className="tiles" size={{ x: 10, y: 10}} flat={false} spacing={1.01} origin={{ x: -100, y: -80 }}>
{
    hexagons.map((hex, i) => (
      <Hexagon
        key={i}
